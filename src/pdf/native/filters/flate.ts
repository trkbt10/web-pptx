...
function decodePngPredictor(
  ...
  if (data.length % rowBytesWithFilter !== 0) {
    throw new Error("Flate predictor: data length is not aligned to PNG predictor rows");
  }
  ...
  for (let y = 0; y < rows; y += 1) {
    ...
    switch (filterType) {
      ...
      case 4: // Paeth
        ...
        break;
      default:
        throw new Error(`Flate predictor: unsupported PNG filter type ${filterType}`);
    }
  }
...
export function decodeFlate(data: Uint8Array, decodeParms?: PdfObject | null): Uint8Array {
  ...
  return applyFlateDecodeParms(decoded, decodeParms);
}
