...
// ISO 32000 PDFDocEncoding mapping table.
// Values are Unicode code points; -1 means "undefined" (fall back to Latin-1 byte value).
const PDF_DOC_ENCODING: readonly number[] = [
...
export function decodePdfDocEncoding(bytes: Uint8Array): string {
  ...
  for (const b of bytes) {
    const mapped = PDF_DOC_ENCODING[b] ?? -1;
    out.push(mapped >= 0 ? mapped : b);
  }
  return String.fromCodePoint(...out);
}

function decodeUtf16Be(bytes: Uint8Array): string {
  // bytes length may be odd; ignore trailing byte.
  const len = bytes.length - (bytes.length % 2);
  const codeUnits: number[] = [];
  for (let i = 0; i < len; i += 2) {
    const hi = bytes[i] ?? 0;
    const lo = bytes[i + 1] ?? 0;
    codeUnits.push((hi << 8) | lo);
  }
...
/**
 * Decode PDF string bytes to JS string.
 *
 * Minimal support:
 * - UTF-16BE/UTF-16LE with BOM (common in Info dictionaries)
 * - Fallback to Latin-1
 */
