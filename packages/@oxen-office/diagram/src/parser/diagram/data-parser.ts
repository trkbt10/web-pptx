...
function parsePointList(
  ...
  if (!element) return [];
  return getChildren(element, "dgm:pt").map((pt) => parsePoint(pt, options));
}

function parsePoint(element: XmlElement, options: DiagramDataModelParseOptions): DiagramPoint {
