...
export type DiagramStyleDefinitionParseOptions = {
  ...
  readonly parseShapeStyle: (element: XmlElement | undefined) => unknown;
};

type Rotation3d = {
  readonly latitude: unknown;
  readonly longitude: unknown;
  readonly revolution: unknown;
};

type BevelPresetType = string;
type PresetMaterialType = string;
type PresetCameraType = string;
type LightRigType = string;
type LightRigDirection = string;
...
export function parseDiagramStyleDefinition(
  doc: XmlDocument,
  options: DiagramStyleDefinitionParseOptions
  ...
  const styleLabels = parseStyleLabels(getChildren(styleDef, "dgm:styleLbl"), options);
...
function parseStyleLabels(
  elements: readonly XmlElement[],
  options: DiagramStyleDefinitionParseOptions
): readonly DiagramStyleLabel[] | undefined {
  if (elements.length === 0) return undefined;
  const labels = elements.map((el) => parseStyleLabel(el, options)).filter((label) => label !== undefined);
...
function parseDiagramScene3d(element: XmlElement | undefined): unknown | undefined {
  if (!element) return undefined;
...
function parseDiagramShape3d(element: XmlElement | undefined): unknown | undefined {
  if (!element) return undefined;
...
function parseBevel(element: XmlElement | undefined): unknown | undefined {
  if (!element) return undefined;
...
function toSolidFill(color: Color | undefined): unknown | undefined {
