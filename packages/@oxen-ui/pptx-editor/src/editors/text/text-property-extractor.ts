...
import type { TextBody, Paragraph, TextRun, RunProperties, ParagraphProperties } from "@oxen-office/pptx/domain/text";
...
export function getRunsInSelection(
  ...
  for (let pIdx = start.paragraphIndex; pIdx <= end.paragraphIndex; pIdx++) {
    ...
    const paragraphRanges = getRunRangesInParagraph({ paragraph, paragraphIndex: pIdx, startOffset: localStart, endOffset: localEnd });
