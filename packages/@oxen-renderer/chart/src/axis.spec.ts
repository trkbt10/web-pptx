...
describe("axis utilities", () => {
  ...
  describe("toLogPosition - ECMA-376 21.2.2.90", () => {
    it("maps minimum value to 0 position", () => {
      ...
      const pos = toLogPosition({ value: 1, minVal: 1, maxVal: 1000, logBase: 10, chartSize: 300 });
      expect(pos).toBeCloseTo(0, 5);
    });

    it("maps maximum value to chart size position", () => {
      const pos = toLogPosition({ value: 1000, minVal: 1, maxVal: 1000, logBase: 10, chartSize: 300 });
    ...
    it("maps middle value correctly for log10 scale", () => {
      ...
      const pos = toLogPosition({ value: 10, minVal: 1, maxVal: 1000, logBase: 10, chartSize: 300 });
    ...
    it("maps value at 100 correctly for log10 scale", () => {
      ...
      const pos = toLogPosition({ value: 100, minVal: 1, maxVal: 1000, logBase: 10, chartSize: 300 });
    ...
    it("handles base 2 logarithms", () => {
      ...
      const pos = toLogPosition({ value: 4, minVal: 1, maxVal: 16, logBase: 2, chartSize: 200 });
    ...
    it("handles natural logarithm (base e)", () => {
      ...
      const pos = toLogPosition({ value: e, minVal: 1, maxVal: e * e, logBase: e, chartSize: 200 });
  ...
  describe("renderMinorGridlines - ECMA-376 21.2.2.99", () => {
    ...
    it("returns empty string when no minor gridlines defined", () => {
      ...
      const result = renderMinorGridlines({ axes: [axis], chartWidth: 300, chartHeight: 200, valueRange: { minVal: 0, maxVal: 100 }, ctx });
      expect(result).toBe("");
    });
    ...
    it("renders minor gridlines for linear scale", () => {
      ...
      expect(result).toContain(`stroke="${ctx.getGridlineColor()}"`);
    ...
    it("renders minor gridlines for log scale base 10", () => {
      ...
      const result = renderMinorGridlines({ axes: [axis], chartWidth: 300, chartHeight: 200, valueRange: { minVal: 1, maxVal: 100 }, ctx });
      expect(result).toContain("<line");
      expect(result).toContain(`stroke="${ctx.getGridlineColor()}"`);
    ...
    it("renders minor gridlines for log scale base 2", () => {
      ...
      const result = renderMinorGridlines({ axes: [axis], chartWidth: 300, chartHeight: 200, valueRange: { minVal: 1, maxVal: 16 }, ctx });
      expect(result).toContain("<line");
  ...
  describe("renderAxisTitles - ECMA-376 21.2.2.211 (title) with c:txPr", () => {
    ...
    it("applies font size from c:txPr defRPr (ECMA-376 21.2.2.217)", () => {
      ...
      const result = renderAxisTitles({ axes: [axis], chartWidth: 300, chartHeight: 200, ctx });
...
describe("renderMultiLevelCategoryAxisLabels (ECMA-376 21.2.2.102 multiLvlStrRef)", () => {
  ...
  it("returns empty string when axis is deleted", () => {
    ...
    const result = renderMultiLevelCategoryAxisLabels({ axes: [axis], chartWidth: 400, chartHeight: 200, multiLevelCategories: multiLevel, ctx });
    expect(result).toBe("");
  });
  ...
  it("handles sparse labels in outer levels", () => {
    ...
    const result = renderMultiLevelCategoryAxisLabels({ axes: [axis], chartWidth: 600, chartHeight: 200, multiLevelCategories: multiLevel, ctx });
    expect(result).toContain("Q1");
    expect(result).toContain("Q2");
  });
