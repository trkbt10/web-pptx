/**
 * @file Slide operations for build command
 *
 * Handles add/remove/reorder/duplicate slide operations directly on ZipPackage.
 * This is a CLI-specific implementation that works with ZipPackage to avoid
 * the PresentationDocument/PresentationFile view conversion issues.
 */
...
import type { SlideAddSpec, SlideRemoveSpec, SlideReorderSpec, SlideDuplicateSpec } from "./types";
import type { Result } from "../../output/json-output";
import { error } from "../../output/json-output";
...
export type SlideOperationsResult<T> =
  ...
  | { readonly success: false; readonly error: Result<T> };
...
export async function applySlideOperations(
  pkg: ZipPackage,
  options: {
    addSlides?: readonly SlideAddSpec[];
    duplicateSlides?: readonly SlideDuplicateSpec[];
    reorderSlides?: readonly SlideReorderSpec[];
    removeSlides?: readonly SlideRemoveSpec[];
  },
  ...
  try {
    ...
    return {
      success: false,
      error: error("SLIDE_OPERATION_ERROR", `Slide operation failed: ${(err as Error).message}`),
    };
  }
}
