...
function extractPptxGradients(xml: string): Array<{ type: string; angle?: number; stops: string[] }> {
...
async function main() {
  ...
  const slideNumber = analyzeAll ? undefined : requireIntArg(flagOrSlide, "slide-number", usage);
  ...
  const startSlide = slideNumber ?? 1;
  const endSlide = slideNumber ?? slideCount;
  ...
  for (let i = startSlide; i <= endSlide; i++) {
    ...
    if (slideXml) {
      const pptxGradients = extractPptxGradients(slideXml);
    ...
    if (themeXml) {
      ...
      if (bgMatch && bgMatch[1].includes("gradFill")) {
        const bgGradients = extractPptxGradients(bgMatch[1]);
