...
function formatTransforms(transforms: readonly ColorTransform[]): string {
  if (transforms.length === 0) {
    return "";
  }
  return ` [${transforms.map((t) => `${t.type}:${t.value}%`).join(", ")}]`;
}
...
async function main() {
  ...
  if (bgMatch) {
    ...
    for (const color of bgColors) {
      console.log(`  ${color.type}:${color.value}${formatTransforms(color.transforms)}`);
    }
