...
function parseArgs(argv: readonly string[]): Args {
  if (argv.includes("--help") || argv.includes("-h")) {
    console.log(
      [
        "Usage:",
        "  bun scripts/update-google-fonts-families.ts --url <metadata-url> --out <output-json> [--versionOut <output-ts>]",
        "",
        "Example:",
        "  bun scripts/update-google-fonts-families.ts --url https://fonts.google.com/metadata/fonts --out pages/public/fonts/google-fonts-families.json --versionOut pages/app/fonts/google-fonts-families.version.ts",
      ].join("\n")
    );
    process.exit(0);
  }
