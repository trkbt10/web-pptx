...
export async function generatePptx(content: SlideContent, outputPath: string): Promise<void> {
  ...
  // Add required files
  pkg.writeText("[Content_Types].xml", CONTENT_TYPES_XML);
  pkg.writeText("_rels/.rels", ROOT_RELS_XML);
  pkg.writeText("ppt/_rels/presentation.xml.rels", PRESENTATION_RELS_XML);
  pkg.writeText("ppt/presentation.xml", PRESENTATION_XML);
  pkg.writeText("ppt/slides/_rels/slide1.xml.rels", SLIDE_RELS_XML);
  pkg.writeText("ppt/slides/slide1.xml", generateSlideXml(content));
